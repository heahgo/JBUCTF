def pad(byte, block_size):
    pad_byte =  bytes({block_size - (len(byte) % block_size)})
    return byte + pad_byte[0] * pad_byte

def unpad(byte, block_size):
    pad_bytes = byte[-1]
    if pad_bytes > block_size or pad_bytes == 0:
        raise ValueError
    for i in range(pad_bytes):
        if byte[-1] != pad_bytes:
            raise ValueError
    return byte[:-(pad_bytes)]

def byte2list(byte):
        return [byte[4*i + j] for i in range(4) for j in range(4)]

def list2byte(arr):
    result = b''
    for i in range(len(arr)):
        result += bytes({arr[i]})
    return result

def list2state(arr):
    return [arr[i+j] for i in range(4) for j in range(0, 16, 4)]
    
def state2list(state):
    return [state[i+j] for i in range(4) for j in range(0, 16, 4)]

def byte2state(byte):
    return list2state(byte2list(byte))

def state2byte(state):
    return list2byte(state2list(state))

def mult(x, y):
    result = 0
    while y:
        if y & 0x01:
            result ^= x
        y >>= 1
        x <<= 1
        if x & 0x100:
            x ^= 0x1b
    return result & 0xff

class myCrypto:
    block_size  = 16
       
    sbox = (
            
            )
    
    inv_sbox = (
            
            )

    def __init__(self, key):
        self.key = key
        if len(key) != 16:
            raise ValueError

    def addRoundKey(self, state):
        key_state = byte2state(self.key)
        return [i^j for i, j in zip(state, key_state)]

    def mixColumns(self, state):
        return [mult(state[i], 2)^mult(state[i+4],3)^state[i+8]^state[i+12] for i in range(4)] + \
               [state[i]^mult(state[i+4], 2) ^mult(state[i+8], 3)^state[i+12] for i in range(4)] + \
               [state[i]^state[i+4] ^mult(state[i+8], 2)^mult(state[i+12], 3) for i in range(4)] + \
               [mult(state[i],3 )^state[i+4]^state[i+8]^mult(state[i+12], 2) for i in range(4)]
    
    def inv_mixColumns(self, state):
        return [mult(state[i], 14)^mult(state[i+4],11)^mult(state[i+8], 13)^mult(state[i+12], 9) for i in range(4)] + \
               [mult(state[i], 9)^mult(state[i+4], 14) ^mult(state[i+8], 11)^mult(state[i+12], 13) for i in range(4)] + \
               [mult(state[i], 13)^mult(state[i+4], 9) ^mult(state[i+8], 14)^mult(state[i+12], 11) for i in range(4)] + \
               [mult(state[i], 11)^mult(state[i+4], 13)^mult(state[i+8], 9)^mult(state[i+12], 14) for i in range(4)]

    def shiftRows(self, state):
        return state[:4] + \
               state[7:8] + state[4:7] + \
               state[10:12] + state[8:10] + \
               state[15:] + state[12:15]
    
    def inv_shiftRows(self ,state):
        return state[:4] + \
               state[5:8] + state[4:5] + \
               state[10:12] + state[8:10] + \
               state[13:] + state[12:13]
    
    def subBytes(self, state):
        return [myCrypto.sbox[i] for i in state]
    
    def inv_subBytes(self, state):
        return [myCrypto.inv_sbox[i] for i in state]
    
    def round(self, state):
        return self.addRoundKey(self.mixColumns(self.shiftRows(self.subBytes(state))))
    def inv_round(self, state):
        return self.inv_subBytes(self.inv_shiftRows(self.inv_mixColumns(self.addRoundKey(state))))
    
    def encrypt(self, state):
        try:
            unpad(state, self.block_size)
        except ValueError as e:
            print(e)
            exit()

        blocks = [state[i: i+16] for i in range(0, len(state), self.block_size)]
        result = b''
        for block in blocks:
            state = byte2state(block)
            for i in range(20):
                state = self.round(state)
            result += state2byte(state)
        return result
            
    def decrypt(self, state):
        if len(state) % self.block_size != 0:
            return 0
        
        blocks = [state[i: i+16] for i in range(0, len(state), self.block_size)]
        result = b''
        for block in blocks:
            state = byte2state(block)
            for i in range(20):
                state = self.inv_round(state)
            result += state2byte(state)
        return result